...
protected
  ...
  String key;
algorithm
  cache := match getGlobalRoot(Global.nfDiagramIconCache)
...
public function clearTopScopeCache
  "Drop everything the last instantiation owns, so a batch caller can release a
   library when it is done with it.

   All three roots are needed. An NF node refers to its enclosing scope weakly
   and a node's children refer to it weakly, so `nfTopScope` and
   `nfIdentityCells` are the only owners of the instantiated classes;
   `instNFNodeCacheIndex` is mkTop's reuse cache. NFInst.makeTopNode resets the
   first two, but only at the *next* instantiation — too late for a caller that
   wants the memory back before it loads the next library."
algorithm
  setGlobalRoot(Global.instNFNodeCacheIndex, {});
