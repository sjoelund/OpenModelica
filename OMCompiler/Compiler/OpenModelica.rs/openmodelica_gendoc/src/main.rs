...
usage: omgendoc [options] LIBRARY...
  ...
  -j, --jobs N           libraries rendered at once, and page-rendering
                         threads (default: the machine's physical cores, since
                         each graphics child holds its own library)
...
struct Options {
    ...
    max_memory: u64,
    /// Set by the parent when it re-runs this executable for one library; the
    /// value is where to write that library's manifest.
    /// How many of the library's classes a predecessor already did.
    stats: bool,
}
...
/// One child per library, so the memory an instantiation does not give back
/// leaves with the process. A child that fails or runs over its timeout costs
/// that library's graphics and nothing else.
/// How many graphics children to run at once.
///
/// Two ceilings, and the lower wins. Cores, because that is what there is to
/// use — physical ones, not every hardware thread: a child is bound by memory
/// traffic rather than arithmetic, so a sibling thread adds little speed and a
/// whole library's memory. And free memory divided by what a child is allowed
/// to hold, because `--max-memory` bounds one child and nothing was bounding
/// their sum: twelve children at six gigabytes is seventy-two, and the machine
/// has thirty-nine. Getting this wrong does not degrade the run, it has the
/// kernel kill it.
...
fn render_graphics(
    ...
    for (done, &root) in libraries.iter().enumerate() {
        ...
        let marker = markers.join(links::file_stem(name));
...
/// goes. Each scope's digests reach `record` as soon as they exist, so a child
