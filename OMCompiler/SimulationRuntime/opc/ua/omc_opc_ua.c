...
readBoolean(UA_Server *server, const UA_NodeId *sessionId,
            void *sessionContext, const UA_NodeId *nodeId,
            void *nodeContext, UA_Boolean includeSourceTimeStamp,
            const UA_NumericRange *range, UA_DataValue *value)
{
  omc_opc_ua_state *state = (omc_opc_ua_state*) nodeContext;
  ...
  value->hasValue = UA_TRUE;
  UA_Variant_setScalarCopy(&value->value, &val, &UA_TYPES[UA_TYPES_BOOLEAN]);
  return UA_STATUSCODE_GOOD;
}

static UA_StatusCode
writeBoolean(UA_Server *server,
          ...
          const UA_NumericRange *range, const UA_DataValue *data)
{
  ...
  if (UA_Variant_isScalar(&data->value) && data->value.type == &UA_TYPES[UA_TYPES_BOOLEAN] && data->value.data) {
    UA_Boolean newVal = *(UA_Boolean*)data->value.data;
...
static UA_StatusCode
readReal(UA_Server *server, const UA_NodeId *sessionId,
         void *sessionContext, const UA_NodeId *nodeId,
         void *nodeContext, UA_Boolean includeSourceTimeStamp,
         const UA_NumericRange *range, UA_DataValue *value)
{
          ...
          const UA_NumericRange *range, const UA_DataValue *data)
{
  ...
  if (!(UA_Variant_isScalar(&data->value) && (data->value.type == &UA_TYPES[UA_TYPES_DOUBLE] || data->value.type == &UA_TYPES[UA_TYPES_FLOAT]) && data->value.data)) {
    BAD_RESULT()
    return UA_STATUSCODE_BADUNEXPECTEDERROR;
  }
  if (data->value.type == &UA_TYPES[UA_TYPES_DOUBLE]) {
    newVal = *(UA_Double*)data->value.data;
  } else {
    newVal = *(UA_Float*)data->value.data;
  }
  pthread_mutex_lock(&state->write_values);
...
static inline omc_opc_ua_state* addVars(omc_opc_ua_state *state, var_kind_t varKind, int n, void *vars, int *varIndex)
{
  ...
  for (i = 0; i < n; i++, (*varIndex)++) {
    ...
    case VARKIND_REAL:
    {
      ...
      dataSource = (UA_DataSource) {.read = readReal, .write = inputIndex >= 0 || isState ? writeReal : NULL};
      break;
    }
    case VARKIND_BOOL:
    {
      ...
      dataSource = (UA_DataSource) {.read = readBoolean, .write = inputIndex >= 0 ? writeBoolean : NULL};
      break;
    }
    default:
    ...
    UA_StatusCode status =
      UA_Server_addDataSourceVariableNode(state->server, nodeId,
                                          ...
                                          name, UA_NODEID_NULL, attr, dataSource, state, NULL);
...
void* omc_embedded_server_init(DATA *data, double t, double step, const char *argv_0, void (*omc_real_time_sync_update)(DATA *data, double scaling), int port)
{
  ...
  state->server = UA_Server_new();
  config = UA_Server_getConfig(state->server);
  ...
  UA_DataSource stepDataSource = (UA_DataSource) {
      .read = readBoolean, .write = writeBoolean};
  UA_VariableAttributes attr;
  UA_VariableAttributes_init(&attr);
  ...
  UA_Server_addDataSourceVariableNode(state->server, stepNodeId,
                                      UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
                                      UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                                      stepName, UA_NODEID_NUMERIC(0, UA_NS0ID_DOUBLE), attr, stepDataSource, state, NULL);
  ...
  UA_Server_addDataSourceVariableNode(state->server, runNodeId,
                                      UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
                                      UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                                      runName, UA_NODEID_NULL, attr, stepDataSource, state, NULL);
  ...
  UA_Server_addDataSourceVariableNode(state->server, timeNodeId,
                                      UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
                                      UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                                      timeName, UA_NODEID_NULL, timeAttr, timeDataSource, state, NULL);
  ...
  state->realVals[0] = malloc(intMin(1,modelData->nVariablesReal) * sizeof(UA_Double));
  state->realVals[1] = malloc(intMin(1,modelData->nVariablesReal) * sizeof(UA_Double));
  state->realValsInputIndex = malloc(intMin(1,modelData->nVariablesReal) * sizeof(int));
  state->boolVals[0] = malloc(intMin(1,modelData->nVariablesBoolean) * sizeof(UA_Boolean));
  state->boolVals[1] = malloc(intMin(1,modelData->nVariablesBoolean) * sizeof(UA_Boolean));
  state->boolValsInputIndex = malloc(intMin(1,modelData->nVariablesBoolean) * sizeof(int));

  state->reinitStateFlag = 0;
  state->stateWasUpdatedFlag = (int*) calloc(sizeof(int), intMin(1,modelData->nStates));
  state->updatedStates = (double*) malloc(sizeof(double)*intMin(1,modelData->nStates));
